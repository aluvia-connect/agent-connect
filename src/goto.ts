...
import { DynamicProxy, ProxySettings } from "./dynamicProxy.js";

const DEFAULT_GOTO_TIMEOUT_MS = 15_000;

const ENV_MAX_RETRIES = Math.max(0, parseInt(process.env.ALUVIA_MAX_RETRIES || "2", 10));
...
export interface AgentConnectRunner {
  goto(url: string, options?: GoToOptions): Promise<{ response: Response | null; page: Page }>
}
...
export interface AgentConnectOptions {
  /** Dynamic proxy to switch upstreams between retries */
  dynamicProxy: DynamicProxy;
  /** Max number of retry attempts after initial failure */
  maxRetries?: number;
  ...
  /** Callback fired after a proxy has been loaded */
...
function compileRetryable(patterns: (string | RegExp)[] = DEFAULT_RETRY_PATTERNS): (err: unknown) => boolean {
  return (err: unknown) => {
    if (!err) return false;
    ...
    return patterns.some((p) =>
      p instanceof RegExp ? p.test(msg) || p.test(code) || p.test(name) : msg.includes(p) || code.includes(p) || name.includes(p)
    );
  };
}
...
export function agentConnectGoTo(page: Page, options?: AgentConnectOptions): AgentConnectRunner {
  const {
    dynamicProxy,
  ...
  const getRawGoto = (p: Page) => ((p as any)[GOTO_ORIGINAL]?.bind(p) ?? p.goto.bind(p)) as Page["goto"];
...
export const agentConnect = agentConnectGoTo;
